...
// FolderUIStateStore persists each account's local folder display state: the user's custom-folder
// order and the collapsed folder paths. This is purely presentational, per-user state (IMAP has no
// folder order of its own), stored durably so it survives application updates.
type FolderUIStateStore interface {
	// FolderUIState returns the account's saved order and collapsed path lists, empty when none saved.
	FolderUIState(ctx context.Context, accountID string) (order []string, collapsed []string, err error)
	// SaveFolderUIState replaces the account's saved order and collapsed path lists.
	SaveFolderUIState(ctx context.Context, accountID string, order, collapsed []string) error
}
...
type MailActions interface {
	...
	// Copy duplicates a message by its opaque handle into the destination mailbox, leaving the original
	// in place. It returns the duplicate's UID in the destination when the server reports it (COPYUID),
	// empty otherwise.
