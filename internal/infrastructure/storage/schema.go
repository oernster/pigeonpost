// Package storage schema: the ordered SQLite migration steps applied by Open, from the initial schema
// to step 14. Kept separate from the store logic in sqlite.go as well as from the later steps in
// schema_steps15.go, migrations_steps30.go and migrations.go, so each file stays within the
// module-size limit.
package storage
