...
// MessageIDs returns the ids of every message cached in a folder. It backs the sync's arrival test,
// which needs membership only, so it reads one indexed column rather than whole summaries.
func (s *Store) MessageIDs(ctx context.Context, folderID string) ([]string, error) {
	return queryRows(ctx, s.db, "message ids",
