...
func (s *Source) FetchRaw(ctx context.Context, account domain.Account, folder domain.Folder, uid uint32) ([]byte, error) {
	client, err := s.connect(ctx, account)
	if err != nil {
		return nil, err
	}
	...
	buffers, err := client.Fetch(uidSet, options).Collect()
	if err != nil {
		return nil, fmt.Errorf("imap: fetch raw uid %d: %w", uid, err)
	}
	if len(buffers) == 0 {
		return nil, fmt.Errorf("imap: message uid %d not found in %q", uid, folder.Path())
	}
	raw := buffers[0].FindBodySection(section)
	if raw == nil {
		return nil, fmt.Errorf("imap: message uid %d has no body section", uid)
	}
	return raw, nil
}
